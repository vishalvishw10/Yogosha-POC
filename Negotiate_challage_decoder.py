...
def request_http(url, virtualhost=None):
  ...
  headers = {'Authorization': 'Negotiate TlRMTVNTUAABAAAAB4IIAAAAAAAAAAAAAAAAAAAAAAA='}
  if virtualhost:
  ...
  if not auth_header:
    print('[!] Negotiate Challenge response not found (WWW-Authenticate header missing)')
    return None

  if not 'Negotiate' in auth_header:
    print('[!] Negotiate Challenge response not found (WWW-Authenticate does not contain "Negotiate")')
    return None
