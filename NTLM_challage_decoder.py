...
def parse_NTLM_flags(NTLM_flags_int):

  flags = OrderedDict()

  flags['NTLMSSP_NTLM_UNICODE']                  = 0x00000001
  flags['NTLM_NTLM_OEM']                         = 0x00000002
  flags['NTLMSSP_REQUEST_TARGET']                     = 0x00000004
  flags['UNUSED_10']                                  = 0x00000008
  flags['NTLMSSP_NTLM_SIGN']                     = 0x00000010
  flags['NTLMSSP_NTLM_SEAL']                     = 0x00000020
  flags['NTLMSSP_NTLM_DATAGRAM']                 = 0x00000040
  flags['NTLMSSP_NTLM_LM_KEY']                   = 0x00000080
  flags['UNUSED_9']                                   = 0x00000100
  flags['NTLMSSP_NTLM_NTLM']                     = 0x00000400
  flags['UNUSED_8']                                   = 0x00000400
  flags['NTLMSSP_ANONYMOUS']                          = 0x00000800
  flags['NTLMSSP_NTLM_OEM_DOMAIN_SUPPLIED']      = 0x00001000
  flags['NTLMSSP_NTLM_OEM_WORKSTATION_SUPPLIED'] = 0x00002000
  flags['UNUSED_7']                                   = 0x00004000
  flags['NTLMSSP_NTLM_ALWAYS_SIGN']              = 0x00008000
  flags['NTLMSSP_TARGET_TYPE_DOMAIN']                 = 0x00010000
  flags['NTLMSSP_TARGET_TYPE_SERVER']                 = 0x00020000
  flags['UNUSED_6']                                   = 0x00040000
  flags['NTLMSSP_NTLM_EXTENDED_SESSIONSECURITY'] = 0x00080000
  flags['NTLMSSP_NTLM_IDENTIFY']                 = 0x00100000
  flags['UNUSED_5']                                   = 0x00200000
  flags['NTLMSSP_REQUEST_NON_NT_SESSION_KEY']         = 0x00400000
  flags['NTLMSSP_NTLM_TARGET_INFO']              = 0x00800000
  flags['UNUSED_4']                                   = 0x01000000
  flags['NTLMSSP_NTLM_VERSION']                  = 0x02000000
  flags['UNUSED_3']                                   = 0x10000000
  flags['UNUSED_2']                                   = 0x08000000
  flags['UNUSED_1']                                   = 0x04000000
  flags['NTLMSSP_NTLM_128']                      = 0x20000000
  flags['NTLMSSP_NTLM_KEY_EXCH']                 = 0x40000000
  flags['NTLMSSP_NTLM_56']                       = 0x80000000

  NTLM_flags = []

  for name,value in flags.items():
    if NTLM_flags_int & value:
      NTLM_flags.append(name)
...
def parse_challenge(challenge_message):
  ...
  # NTLMFlags
  NTLM_flags_int = decode_int(challenge_message[20:24])

  NTLM_flags = parse_NTLM_flags(NTLM_flags_int)
...
def print_challenge(challenge):
  ...
  if 'NTLMSSP_TARGET_TYPE_DOMAIN' in challenge['NTLM_flags']:
    print('\nTarget (Domain): {}'.format(challenge['target_name']))
  elif 'NTLMSSP_TARGET_TYPE_SERVER' in challenge['NTLM_flags']:
  ...
  for flag in challenge['NTLM_flags']:
    print('  {}'.format(flag))
...
def request_SMBv23(host, port=445):
  ...
  if smb_client.RequireMessageSigning is True:
    session_setup['SecurityMode'] = smb3.SMB2_NTLM_SIGNING_REQUIRED
  else:
    session_setup['SecurityMode'] = smb3.SMB2_NTLM_SIGNING_ENABLED
...
def request_mssql(host, port=1443):
    ...
    # Send the NTLMSSP NTLM or SQL Auth Packet
